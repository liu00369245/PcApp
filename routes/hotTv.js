...
router.get('/getData', function (req, res, next) {
    saveIdt = [];
    http.get('127.0.0.1:3333/hotTv/find', { submitType: 'findJoin', ref: ['details', ''] }).then(function (data) {
        let arr = [];
        for (let item of data) {

            saveIdt.push(item.details[0]._id)
...
router.get('/findData', function (req, res, next) {
    http.get('127.0.0.1:3333/details/find', req.query).then(function (data) {
        for (let i = 0; i < data.rows.length; i++) {
            for (let j = 0; j < saveIdt.length; j++) {
                if (data.rows[i]._id == saveIdt[j]) {
...
router.post('/addData', function (req, res, next) {
    ...
    http.post('127.0.0.1:3333/hotTv/add', { details: [idd] }).then(function (data) {
        res.send(data)
    })
});
// 删除查询
router.post('/delData', function (req, res, next) {
    ...
    http.post('127.0.0.1:3333/hotTv/del', { ids: aa }).then(function (data) {
        res.send(data)
    })
});
// 搜索
router.get('/search', function (req, res, next) {
    ...
    http.post('127.0.0.1:3333/details/find', req.query).then(function (data) {
        // 判断自己数据库是否有
        http.get('127.0.0.1:3333/hotTv/find', {}).then(function (newData) {
            let obj = [];
